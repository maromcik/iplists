...
<div class="w-full max-w-4xl mx-auto p-4">
    {#if !locationValue || !locationType}
        <ErrorAlert error={{ code: 400, kind: AppErrorKind.BadRequest, description: "Missing country or continent parameter" }} title="Invalid request" />
    {:else}
        ...
        {#if loading}
            <p class="text-gray-600 dark:text-gray-400">Loading ranges…</p>
        {:else if error}
            <ErrorAlert error={error} title="Could not load IP ranges" />
        {:else}
            <div class="mb-6 p-4 bg-gray-100 dark:bg-gray-800 rounded-xl">
